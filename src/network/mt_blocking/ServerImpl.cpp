...
void ServerImpl::Stop() {
    ...
    std::unique_lock<std::mutex> lg1(_set_is_blocked);

    for (std::set<int>::iterator it = _client_descriptors.begin(); it != _client_descriptors.end(); ++it)
        shutdown(*it, SHUT_RD);
}
...
void ServerImpl::Join() {
    ...
    _thread.join();
    std::unique_lock<std::mutex> lk(_one_thread_stopped);
    while (_workers != 0)
        _alive_workers_number.wait(lk);
}
...
void ServerImpl::OnRun() {
    ...
    while (_running.load()) {
        ...
        //    _workers.store(0>debug("waiting for connection...");
        ...
        if (_workers < max_workers && _running.load()) {
            ++_workers;
            std::thread(&ServerImpl::user_handler, this, client_socket).detach();
            {

                std::lock_guard<std::mutex> lg1(_set_is_blocked);

                _client_descriptors.insert(client_socket);
            }
        }
    }
...
void ServerImpl::user_handler(int client_socket) {
    ...
    --_workers;
    if (_workers == 0) {
        _alive_workers_number.notify_all();
    }
